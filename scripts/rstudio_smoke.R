...
run_rstudio_smoke <- function(project_root, show_data = TRUE) {
  ...
  # Restore Python-side settings before the final two-layer R RNG restore.
  # A later reticulate call must not reopen an R RNG scope after that restore.
  on.exit(torch$set_num_threads(as.integer(previous_threads)), add = TRUE, after = FALSE)
  set.seed(20260926)

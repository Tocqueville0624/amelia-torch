...
- [ ] 在发布支持的 GPU 精度下对新增初值/迭代边界与病态状态做有界验证；[MPS32 八案例](validation/2026-09-26-g2/accelerator-edges-mps32.md)已完成，CUDA32/64 已排队待执行。MPS 的 13 次 EM 中两次预定截止/病态未收敛被正确报告，病态伪逆和 autopri 轨迹差异全部保留；不把边界断言通过说成每次插补质量成功。

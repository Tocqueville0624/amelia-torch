...
测试套件会拒绝已有输出目录，并在启动子任务前保存完整计划和当前状态。Python 基准逐次以临时文件替换完整 JSON，保留已完成的重复；可捕获的键盘中断记为 `interrupted`，未知子进程退出值保留 `null`。预热失败也不能报告“全部成功”。这些措施不改变拟合参数、随机种子或计时范围，也不自动续跑；强制终止、掉电或云端虚拟机被回收仍可能只留下最后一份快照。云端阶段完成后还须导出检查点到本地，不能把虚拟机内文件当成外部备份。
